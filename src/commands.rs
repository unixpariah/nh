...
/// - `Force(&'static str)`: use only the specified program, error if not available.
#[allow(dead_code)]
...
impl ElevationStrategy {
    pub fn resolve(&self) -> Result<PathBuf> {
        match self {
            ...
            ElevationStrategy::Prefer(program) => which(program).or_else(|_| {
    ...
    /// Gets a path to a privilege elevation program based on what is available in the system.
    ///
    ...
    /// # Returns
    ///
    /// * `Result<PathBuf>` - The absolute path to the privilege elevation program binary or an error if a
